...
data_cleaned <- readRDS(paste0(mainDir, subDir, name_df, "_clean", 'Example.rds')) 
...
* firms for which the [sum of business segment sales or assets] deviates from the [firm’s total sales or assets] by [more than 5%]
are also excluded, 
...
# readRDS(paste0(Sys.getenv("Data"), "/Compustat/", "inferEarnA_raw", "2019-11-04_01.36.36", '.rds')) %>% 
#   filter(fyear >= 2001 & fyear <= 2005) %>% 
#   saveRDS(paste0(Sys.getenv("Data"), "/Compustat/", "inferEarnA_raw", '2001-2005.rds'))

# The raw Fundamental Annual is most complete as it's not subject to prior deficiency in handling data


dDiscA <- read.csv(paste0(mainDir, subDir, 'ccmfunda_01Jan1990_31Dec1991.csv'), 
                   stringsAsFactors = FALSE) %>% 
# For simplicity, keep only the security issue ID = 01 so that 
#   there is only one issue for each firm in the sample
  filter(LIID == "1") %>% 
  left_join(
    read.csv(paste0(mainDir, subDir, 'ccmfunda_LIID=01_(FIC_ADRR)_01Jan1990_31Dec1991.csv'),
             stringsAsFactors = FALSE) %>% 
      select(GVKEY, datadate, fic, adrr), 
    by = c("GVKEY", "datadate")  # note: gvkey originally was in captial letters after download
    ) %>%  
...
names(dDiscA) <- tolower(names(dDiscA))  
  


#dDiscA <- readRDS(paste0(Sys.getenv("Data"), "/Segment/", "inferEarnA_raw", '2001-2005.rds')) %>% 
dDiscA <- dDiscA %>% 
...
startdate = "1990-01-01" #"1998-01-01" 
enddate = "1999-12-31"    #"1999-12-31"
